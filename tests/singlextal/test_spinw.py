...
class TestCase(ut.TestCase):
    ...
    def test_get_dispersions_along_slice(self):
        spinw_qs, omega, swInt \
            = spinw.get_dispersions_along_slice(ml_slice_func, slice=Slice_00L, Nq_disp=500)
        if plot:
    ...
    def test_get_thin_slice(self):
        qmg, Emg, slice_img = spinw.get_thin_slice(ml_slice_func, slice=Slice_00L, branches=[0])
        if plot:
            from matplotlib import pyplot as plt
    ...
    def test_get_slice(self):
        qmg, Emg, slice_img = spinw.get_slice(ml_slice_func, slice=Slice_00L, branches=[0])
        if plot:
            from matplotlib import pyplot as plt
            plt.pcolormesh(qmg, Emg, slice_img.T)
            plt.colorbar()
