...
class Fit2EE(fit_2d_psf.Fit):
    ...
    def fit_E_profile(self, sigma_left, sigma_right, weight_left, ef_width):
        # fit res(E)
        ...
        if isinstance(sigma_left, tuple):
            # inputs are bounds
            bounds = [sigma_left, sigma_right, weight_left, ef_width]
            x0 = [np.average(b) for b in bounds]
        else:
            # inputs are single numbers
            x0 = [sigma_left, sigma_right, weight_left, ef_width]
            bounds = np.array([
                [sigma_left/3, sigma_right/3, 0, ef_width/10.],
                [sigma_left*3, sigma_right*3, 1., ef_width*10.]
            ]).T
...
class EEAffineModel(fit_2d_psf.PSF_Affine_Model):
    ...
    def update_parameters(self, sigma_left=None, sigma_right=None, weight_left=None, ef_width=None, q_sigma=None, dq_over_dE=None):
        self.set_E_profile(sigma_left, sigma_right, weight_left, ef_width)
        self.set_q_profile(q_sigma)
        self.set_qE_profile(dq_over_dE)
        return

    def set_E_profile(self, sigma_left, sigma_right, weight_left, ef_width):
        self.E_profile = self.make_E_profile_function(sigma_left, sigma_right, weight_left, ef_width)
        return self.E_profile

    def make_E_profile_function(self, sigma_left, sigma_right, weight_left, ef_width):
        return EEModel(sigma_left, sigma_right, weight_left, ef_width)
